...
export async function GET(
  ...
  try {
    if (lithuania.length === 0) {
      return new NextResponse("No data found", { status: 404 });
    }

    switch (fileType) {
      case "csv": {
        const fields = Object.keys(lithuania[0]);
        const parser = new Json2csvParser({ fields });
        const csv = parser.parse(lithuania);

        return new NextResponse(csv, {
          status: 200,
          headers: {
            "Content-Type": "text/csv",
            "Content-Disposition": 'attachment; filename="lithuania.csv"',
          },
        });
      }

      case "xlsx": {
        const worksheet = XLSX.utils.json_to_sheet(lithuania);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "lithuania");
        ...
        return new NextResponse(buffer, {
          ...
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="lithuania.xlsx"',
          },
        });
      }
      ...
      case "pdf": {
        ...
        return new NextResponse(Buffer.from(pdfBytes), {
          ...
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="lithuania.pdf"',
          },
        });
      }
