  ...
  try {
    ...
    switch (fileType) {
      ...
      case "pdf": {
        ...
        const fieldsToInclude = ["FirmName", "EmoneyRegisterStatus", "EmoneyStatusEffectiveDate"];
        const headersPretty = ["Firm Name", "Emoney Register Status", "Emoney Status Effective Date"];

        const slimData = eMoneyFirms.map((row) => {
        ...
        for (const row of slimData) {
          const wrapped = [
            ...
            wrapText(String(row["EmoneyRegisterStatus"]), addressWidth, customFont, fontSize),
            wrapText(String(row["EmoneyStatusEffectiveDate"]), addressTypeWidth, customFont, fontSize),
          ];
