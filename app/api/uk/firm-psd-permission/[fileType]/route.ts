...
export async function GET(req: NextRequest, { params }: { params: { fileType: string } }) {
  ...
  try {
    ...
    switch (fileType) {
      case "csv": {
        const fields = Object.keys(firmPSDPermission[0]);
        const parser = new Json2csvParser({ fields });
        const csv = parser.parse(firmPSDPermission);

        return new NextResponse(csv, {
          status: 200,
          headers: {
            "Content-Type": "text/csv",
            "Content-Disposition": 'attachment; filename="firmPSDPermission.csv"',
          },
        });
      }

      case "xlsx": {
        const worksheet = XLSX.utils.json_to_sheet(firmPSDPermission);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "firmPSDPermission");
        ...
        return new NextResponse(buffer, {
          ...
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="firmPSDPermission.xlsx"',
          },
        });
      }
      ...
      case "pdf": {
        ...
        const fieldsToInclude = ["FirmName", "PSDFirmStatus", "PSDStatusEffectiveDate"];
        const headersPretty = ["Firm Name", "PSD Firm Status", "PSD Status Effective Date"];

        const slimData = firmPSDPermission.map((row) => {
        ...
        for (const row of slimData) {
          const wrapped = [
            ...
            wrapText(String(row["PSDFirmStatus"]), addressWidth, customFont, fontSize),
            wrapText(String(row["PSDStatusEffectiveDate"]), addressTypeWidth, customFont, fontSize),
          ];
